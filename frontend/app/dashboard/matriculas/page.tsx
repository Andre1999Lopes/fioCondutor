...
function MatriculasContent() {
  ...
  // Mapear matrículas com nomes de alunos e turmas
  const matriculasComNomes = matriculas?.map((mat: Matricula) => {
  ...
  const filteredMatriculas = matriculasComNomes?.filter((mat) => {
    const matchSearch =
      search === '' ||
      mat.alunoNome?.toLowerCase().includes(search.toLowerCase()) ||
      false ||
      mat.turmaNome?.toLowerCase().includes(search.toLowerCase()) ||
      false;
    const matchStatus = filterStatus === 'todas' || (filterStatus === 'ativas' ? mat.ativa : !mat.ativa);
        ...
        <table className='w-full'>
          <thead className='bg-gray-50 border-b'>
            <tr>
              ...
              <th className='px-6 py-3 text-left text-sm font-medium text-gray-700'>Data de Matrícula</th>
              <th className='px-6 py-3 text-left text-sm font-medium text-gray-700'>Status</th>
              <th className='px-6 py-3 text-center text-sm font-medium text-gray-700'>Ações</th>
            </tr>
          </thead>
            ...
            {isLoading ? (
              ...
              filteredMatriculas.map((mat: Matricula) => (
                <tr key={mat.id} className='hover:bg-gray-50'>
                  <td className='px-6 py-4 text-sm font-medium text-gray-900'>{mat.alunoNome}</td>
                  <td className='px-6 py-4 text-sm text-gray-600'>{mat.turmaNome}</td>
                  ...
                  <td className='px-6 py-4'>
                    <span
                      className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${
                        mat.ativa ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                      }`}
                    >
